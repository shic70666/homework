...
for ch_num in range(0, len(data_case)):
    name = 'CH{}'.format(ch_num+1)
    each_channel = pd.DataFrame( columns=[name] )
    ...
    for i in range(0, len(data_case[ch_num])//ns):
        ...
        each_channel = each_channel.append([{name:frame_temp,
                                             }], ignore_index=True)

    each_case = pd.concat([each_case, each_channel], axis=1)

    filename = dataFile.stem
    each_case.to_json(filename+".json")

# Version 1: generate .csv files
# case_num = 1
